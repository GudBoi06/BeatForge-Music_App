...
// Ensures every kit has isolated memory
const getInitializedKits = (sourceKits, steps = 16) => {
  return sourceKits.map(kit => {
    const soundCount = kit.sounds.length;
    return {
      ...kit,
      grid: kit.grid || Array.from({ length: soundCount }, () => Array(steps).fill(false)),
      volumes: kit.volumes || Array(soundCount).fill(1),
      mutedTracks: kit.mutedTracks || Array(soundCount).fill(false)
    };
  });
};
...
export default function StepSequencer({ 
  ...
  // Master State: Holds all isolated kit data
  const [kits, setKits] = useState(() => {
    try {
      const saved = localStorage.getItem("beatforge_seq_session");
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed.kits) return getInitializedKits(parsed.kits, parsed.stepsCount || 16);
      }
    } catch(e) {}
    return getInitializedKits(initialDrumKits, 16);
  });
  ...
  // Extract current isolated workspace
  ...
  // Proxy setters for isolated Mixer updates
  const handleSetVolumes = (newVolumes) => {
    setKits(prev => {
      const copy = [...prev];
      const targetKit = { ...copy[currentKitIndex] };
      targetKit.volumes = typeof newVolumes === "function" ? newVolumes(targetKit.volumes) : newVolumes;
      copy[currentKitIndex] = targetKit;
      return copy;
    });
  };

  const handleSetMutedTracks = (newMutes) => {
    setKits(prev => {
      const copy = [...prev];
      const targetKit = { ...copy[currentKitIndex] };
      targetKit.mutedTracks = typeof newMutes === "function" ? newMutes(targetKit.mutedTracks) : newMutes;
      copy[currentKitIndex] = targetKit;
      return copy;
    });
  };
  ...
  const handleKitChange = (direction) => {
    setCurrentKitIndex(prev => direction === -1 ? (prev === 0 ? kits.length - 1 : prev - 1) : (prev + 1) % kits.length);
  };
    ...
    if (prevMySamplesRef.current && prevMySamplesRef.current.length === mySamples.length + 1) {
      ...
      if (deletedSampleId) {
        setKits(prev => {
          ...
          copy.forEach((kit, kitIdx) => {
            if (kit.sounds) {
              const indicesToRemove = [];
              kit.sounds.forEach((sound, index) => { if (String(sound.sourceId) === String(deletedSampleId)) indicesToRemove.push(index); });
              if (indicesToRemove.length > 0) {
                hasChanges = true;
                copy[kitIdx] = {
                  ...kit,
                  sounds: kit.sounds.filter((_, i) => !indicesToRemove.includes(i)),
                  grid: kit.grid.filter((_, i) => !indicesToRemove.includes(i)),
                  volumes: kit.volumes.filter((_, i) => !indicesToRemove.includes(i)),
                  mutedTracks: kit.mutedTracks.filter((_, i) => !indicesToRemove.includes(i))
                };
              }
            }
          });
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.kits && setHasActiveBeat) {
          const currentLoadedKit = parsed.kits[parsed.kitIndex || 0];
          const hasNotes = currentLoadedKit.grid && currentLoadedKit.grid.some(row => row && row.some(step => step === true));
  ...
  // Crash-Proof Local Storage Saver
  useEffect(() => {
    if (!isLoaded) return;
    try {
      const session = { bpm, stepsCount: Number(stepsCount), kitIndex: currentKitIndex, kits };
      ...
      try {
        const lightweightKits = kits.map(kit => ({
          ...
          sounds: kit.sounds.map(sound => {
            if (sound.file && sound.file.length > 500) return { ...sound, file: null, name: sound.name + " (Unsaved)" };
            return sound;
          })
        }));
        const lightweightSession = { bpm, stepsCount: Number(stepsCount), kitIndex: currentKitIndex, kits: lightweightKits };
  ...
  useEffect(() => {
    ...
    if (currentUser?._id !== prevUserRef.current?._id) {
      setKits(getInitializedKits(initialDrumKits, Number(stepsCount)));
  ...
  const playDrumSample = (fileUrl, trackVolume) => {
    ...
    sourceNode.onended = () => { activeAudioNodesRef.current = activeAudioNodesRef.current.filter(n => n !== sourceNode); };
  };
  ...
  const updateStep = (row, step, value) => {
    ...
    setKits(prev => {
      const copy = [...prev];
      const kit = { ...copy[currentKitIndex] };
      const newGrid = kit.grid.map(r => [...(r || [])]);
      if (!newGrid[row]) newGrid[row] = Array(Number(stepsCount)).fill(false);
      newGrid[row][step] = value;
      kit.grid = newGrid;
      copy[currentKitIndex] = kit;
      return copy;
    });
  };
  ...
  const clearBeat = () => {
    setKits(prev => {
      const copy = [...prev];
      const kit = { ...copy[currentKitIndex] };
      kit.grid = kit.grid.map(() => Array(Number(stepsCount)).fill(false));
      copy[currentKitIndex] = kit;
      return copy;
    });
    setCurrentStep(0);
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onloadend = () => {
      ...
      setKits(prev => {
        const copy = [...prev];
        const kit = { ...copy[currentKitIndex] };
        kit.sounds = [...kit.sounds, newSound];
        kit.grid = [...kit.grid, Array(Number(stepsCount)).fill(false)];
        kit.volumes = [...kit.volumes, 1];
        kit.mutedTracks = [...kit.mutedTracks, false];
        copy[currentKitIndex] = kit;
        return copy;
      }); 
    };
  ...
  const addTrackFromBrowser = (sample) => {
    if (!sample) return;
    const newSound = { name: sample.name.substring(0, 10).toUpperCase(), file: sample.file, sourceId: String(sample.id) };
    setKits(prev => {
      const copy = [...prev];
      const kit = { ...copy[currentKitIndex] };
      kit.sounds = [...kit.sounds, newSound];
  ...
  const deleteTrack = (rowIndex) => {
    ...
    setKits(prev => {
      const copy = [...prev];
      const kit = { ...copy[currentKitIndex] };
      kit.sounds = kit.sounds.filter((_, i) => i !== rowIndex);
      kit.grid = kit.grid.filter((_, i) => i !== rowIndex);
      kit.volumes = kit.volumes.filter((_, i) => i !== rowIndex);
      kit.mutedTracks = kit.mutedTracks.filter((_, i) => i !== rowIndex);
      copy[currentKitIndex] = kit;
      return copy;
    });
  };
  ...
  const savePreset = async () => {
    ...
    const presetData = { name: presetName, bpm, stepsCount: Number(stepsCount), grid, volumes, mutedTracks, kitIndex: currentKitIndex, sounds: currentSoundsRef.current };
  ...
  const loadPreset = (preset) => {
    ...
    setKits(prev => {
      const copy = [...prev];
      const targetKit = { ...copy[targetIndex] };
      if (preset.sounds && preset.sounds.length > 0) targetKit.sounds = preset.sounds;
      targetKit.grid = preset.grid || targetKit.sounds.map(() => Array(Number(preset.stepsCount) || 16).fill(false));
      targetKit.volumes = preset.volumes || targetKit.sounds.map(() => 1);
      targetKit.mutedTracks = preset.mutedTracks || targetKit.sounds.map(() => false);
      copy[targetIndex] = targetKit;
      return copy;
    });
  ...
  const initiateDeletePreset = () => {
    ...
    if (selectEl && selectEl.value !== "") setPresetToDeleteIndex(Number(selectEl.value));
  };
  ...
  const fillSteps = (rowIndex, interval, startStep) => {
    ...
    setKits(prev => {
      ...
      const kit = { ...copy[currentKitIndex] };
      const newGrid = kit.grid.map(r => [...(r || [])]);
      if (newGrid[rowIndex]) newGrid[rowIndex] = newGrid[rowIndex].map((_, i) => i >= startStep && (i - startStep) % interval === 0);
      kit.grid = newGrid;
      copy[currentKitIndex] = kit;
      return copy;
    });
  ...
  const clearTrack = (rowIndex) => {
    ...
    setKits(prev => {
      ...
      const kit = { ...copy[currentKitIndex] };
      const newGrid = kit.grid.map(r => [...(r || [])]);
      if (newGrid[rowIndex]) newGrid[rowIndex] = newGrid[rowIndex].map(() => false);
      kit.grid = newGrid;
      copy[currentKitIndex] = kit;
      return copy;
    });
  ...
  const confirmSaveToLivePad = () => {
    ...
    const newPattern = {
      id: `drum-${Date.now()}`, type: 'drum', name: finalName, stepsCount: Number(stepsCount),
      ...
      {showLivePadModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#111', padding: '30px', borderRadius: '12px', border: '1px solid #333', textAlign: 'center', maxWidth: '350px', width: '100%', boxShadow: '0 10px 40px rgba(0,0,0,0.8)' }}>
            ...
            <input autoFocus type="text" value={livePadName} onChange={(e) => setLivePadName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && confirmSaveToLivePad()} className="led-input" style={{ width: '100%', marginBottom: '20px', boxSizing: 'border-box', textAlign: 'center', fontSize: '16px', padding: '10px' }} />
      ...
      {showProModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#111', padding: '30px', borderRadius: '12px', border: '1px solid #333', textAlign: 'center', maxWidth: '350px', boxShadow: '0 10px 40px rgba(0,0,0,0.8)' }}>
            ...
            <p style={{ color: '#aaa', fontSize: '14px', lineHeight: '1.5', marginBottom: '24px' }}>This feature is exclusive to Beatforge Pro.<br/><br/>Click the <strong style={{color: '#FFD700'}}>⭐ UNLOCK PRO</strong> button in the top bar to get lifetime access!</p>
      ...
      {presetToDeleteIndex !== null && presets[presetToDeleteIndex] && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#111', padding: '30px', borderRadius: '12px', border: '1px solid #333', textAlign: 'center', maxWidth: '350px', boxShadow: '0 10px 40px rgba(0,0,0,0.8)' }}>
            ...
            <p style={{ color: '#aaa', fontSize: '14px', lineHeight: '1.5', marginBottom: '24px' }}>Are you sure you want to permanently delete the preset <br/><strong style={{color: '#fff'}}>"{presets[presetToDeleteIndex].name}"</strong>?</p>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
      ...
      <div className="sequencer" onContextMenu={(e) => e.preventDefault()}>
        <div className="preset-bar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          <div className="sequencer-header">
            <div className="header-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="3" y1="9" x2="21" y2="9"></line><line x1="9" y1="21" x2="9" y2="9"></line></svg></div>
          ...
          <div className="hardware-panel">
            ...
            <div className="panel-controls">
              <button className="panel-btn" onClick={() => handleKitChange(-1)} title="Previous Kit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></button>
              <div className="led-input" style={{ width: '150px', textAlign: 'center', userSelect: 'none', color: '#00e676', textShadow: '0 0 5px rgba(0, 230, 118, 0.5)' }}>
                {currentKit.name} {currentKit.proOnly && "👑"}
              </div>
              <button className="panel-btn" onClick={() => handleKitChange(1)} title="Next Kit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></button>
            </div>
          </div>
          ...
          <div className="hardware-panel">
            ...
            <div className="panel-controls">
              ...
              <button onClick={savePreset} className="panel-btn action-btn" title="Save Preset" style={{ background: ["no_name", "no_user", "error"].includes(presetSaveStatus) ? '#ff4757' : (presetSaveStatus === "saved" ? '#00e676' : 'transparent'), color: presetSaveStatus !== "idle" ? (presetSaveStatus === "saved" ? '#000' : '#fff') : 'var(--text-light)', border: presetSaveStatus !== "idle" ? 'none' : '1px solid var(--border-dark)', transition: 'all 0.2s', minWidth: '85px', justifyContent: 'center', fontWeight: presetSaveStatus !== "idle" ? 'bold' : 'normal' }}>
                {presetSaveStatus === "idle" && (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '6px' }}><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg>)}
                {presetSaveStatus === "idle" ? "Save" : presetSaveStatus === "no_name" ? "⚠️ NAME" : presetSaveStatus === "no_user" ? "⚠️ LOGIN" : presetSaveStatus === "saved" ? "✔️ SAVED" : "⚠️ ERROR"}
              </button>
              ...
              <button className="panel-btn danger-icon-btn" title="Delete Preset" onClick={initiateDeletePreset}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2-2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
            </div>
          </div>
          ...
          <div className="hardware-panel">
            ...
            <div className="panel-controls grid-tools-group">
              ...
              <div className="trigger-pad-wrapper">
                ...
                <button onClick={clearBeat} className="trigger-pad" title="Clear Grid"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path></svg></button>
                <div className="pad-spacer"></div>
              </div>
              <div className="panel-divider"></div>
              <button className="panel-btn action-btn" onClick={() => setShowMixer(!showMixer)} title="Toggle Mixer Console" style={{ background: showMixer ? 'var(--accent)' : 'transparent', color: showMixer ? '#fff' : 'var(--text-light)' }}><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '6px' }}><path d="M4 22v-7"></path><path d="M4 11V2"></path><path d="M20 22v-5"></path><path d="M20 13V2"></path><path d="M12 22v-11"></path><path d="M12 7V2"></path><line x1="2" y1="11" x2="6" y2="11"></line><line x1="10" y1="7" x2="14" y2="7"></line><line x1="18" y1="13" x2="22" y2="13"></line></svg>MIXER</button>
              <button onClick={initiateSaveToLivePad} className="panel-btn action-btn" style={{ fontWeight: 'bold', background: saveStatus === 'empty' ? '#ff4757' : (saveStatus === 'saved' ? '#00e676' : 'var(--accent)'), color: '#fff', border: 'none', marginLeft: '10px', transition: 'all 0.2s', width: '130px', justifyContent: 'center' }}>{saveStatus === 'empty' ? '⚠️ EMPTY' : saveStatus === 'saved' ? '✔️ SAVED' : '💾 TO LIVE PAD'}</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', minHeight: isLocked ? '250px' : 'auto' }}>
          {isLocked && (
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(5px)', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '8px', border: '1px solid rgba(255, 215, 0, 0.2)' }}>
              ...
              <p style={{ color: '#ddd', margin: 0, fontSize: '14px' }}>The <strong>{currentKit.name}</strong> kit is exclusive to Pro users.</p>
          ...
          {!showMixer ? (
                ...
                return (
                  <div key={`${safeName}-${rowIndex}`} className={`sequencer-row ${mutedTracks[rowIndex] ? "muted" : ""}`}>
                    <div className="track-label">
                      ...
                      <div className="track-controls"><button className="track-options-btn" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setContextMenu({ visible: true, x: e.clientX, y: e.clientY, rowIndex: rowIndex, startStep: 0 }); }}><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="1"></circle><circle cx="12" cy="5" r="1"></circle><circle cx="12" cy="19" r="1"></circle></svg></button></div>
                    </div>
                    <div className="steps" onMouseUp={() => (dragMode.current = null)} onMouseLeave={() => (dragMode.current = null)}>
                      {(grid[rowIndex] || Array(Number(stepsCount)).fill(false)).map((active, stepIndex) => {
                        ...
                        return (
                          <div key={stepIndex} className={`step ${active ? "active" : ""} ${currentStep === stepIndex ? "current" : ""} ${isAltGroup ? "alt-group" : ""}`} onMouseDown={(e) => { isDraggingRef.current = false; if (e.button === 2) { dragMode.current = "erase"; updateStep(rowIndex, stepIndex, false); } else { dragMode.current = "paint"; updateStep(rowIndex, stepIndex, true); } }} onMouseEnter={() => { if (!dragMode.current) return; isDraggingRef.current = true; updateStep(rowIndex, stepIndex, dragMode.current === "paint"); }} onContextMenu={(e) => e.preventDefault()} />
                        );
                      })}
                    </div>
                  </div>
            ...
            <Mixer currentSounds={currentSounds} volumes={volumes} setVolumes={handleSetVolumes} mutedTracks={mutedTracks} setMutedTracks={handleSetMutedTracks} />
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
          ...
          <button className="panel-btn" onClick={() => { if (currentUser?.isPro) { fileInputRef.current.click(); } else { setShowProModal(true); } }} style={{ flex: 1, padding: '12px', borderStyle: 'dashed', borderColor: 'rgba(255, 255, 255, 0.2)', justifyContent: 'center', letterSpacing: '2px', color: 'var(--text-muted)' }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '8px', width: '16px' }}><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>UPLOAD SAMPLE {currentUser?.isPro ? "" : "👑"}
          </button>
          <select className="panel-btn" style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px dashed rgba(166, 74, 255, 0.4)', color: 'var(--accent)', cursor: 'pointer', outline: 'none', letterSpacing: '2px', textAlign: 'center', appearance: 'none' }} onChange={(e) => { const selectedId = e.target.value; const sample = mySamples.find(s => String(s.id) === String(selectedId)); if (sample) addTrackFromBrowser(sample); e.target.value = ""; }}>
